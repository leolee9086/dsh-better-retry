...
const zh = { add: '纳入自动重试', added: '已纳入自动重试', saving: '正在保存…', error: '保存失败' }
const en = { add: 'Enable automatic retry', added: 'Automatic retry enabled', saving: 'Saving…', error: 'Save failed' }
  ...
  const add = async () => {
    ...
    try {
      ...
      if (!current.includes(node.message)) {
        await remote.settings.update(NS, { rules: [...current, node.message] }, view?.revision)
      }
      setState('added')
    } catch (_error) {
      setState('error')
